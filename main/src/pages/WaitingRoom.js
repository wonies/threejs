...
export default class WaitingRoom extends InGamePage {
    setup() {
        ...
        // sessionStorage에서 저장된 이름을 가져옵니다.
        const players = JSON.parse(sessionStorage.getItem('players') || '{}');
        
        // 실제 저장된 플레이어 이름만 가져옵니다.
    ...
    template() {
        ...
        const playerList = playerNames.map((name, index) => `
    ...
    setEvent() {
        ...
        $target.addEventListener('click', ({ target }) => {
            if (target.matches('.start-game') || target.matches('.start-tournament')) {
